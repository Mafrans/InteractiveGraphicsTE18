    ...
    private void update() {
        ...
        for (int i = 0; i < rider.shadow.getHeight(); i++) {
            for (int j = 0; j < rider.shadow.getWidth(); j++) {
                ...
                if (distance(j / 2 + rider.shadow.getWidth() / 4, i, rider.shadow.getWidth() / 2, rider.shadow.getWidth() / 2) < rider.shadow.getHeight() / 4) {
        ...
        for (int i = 0; i < rider.getHeight(); i++) {
            for (int j = 0; j < rider.getWidth(); j++) {
                ...
                if (index < 0 || index > pixels.length - 1) continue;
                int color = rider.getPixels()[i * rider.getWidth() + j];

                if (color != -16777216) { // Alpha mask
                    pixels[index] = color;
                }
            }
        }
